export interface Project {
  slug: string;
  name: string;
  nameBn: string;
  location: string;
  locationBn: string;
  city: "Dhaka" | "Chandpur";
  status: "ongoing" | "upcoming" | "completed";
  flatSize: string;
  landArea: string;
  floors: string;
  facing: string;
  amenities: string[];
  description: string;
  descriptionBn: string;
  image: string;
  images?: string[];
  brochure?: string;
}

export const projects: Project[] = [
  {
    slug: "bondhon-tower",
    name: "Bondhon Tower",
    nameBn: "বন্ধন টাওয়ার",
    location: "Anando Nagar, Jheelpar, Hazi Wakiluddin Road, Merul Badda, Dhaka",
    locationBn: "আনন্দ নগর, ঝিলপাড়, হাজি ওয়াকিলউদ্দিন রোড, মেরুল বাড্ডা, ঢাকা",
    city: "Dhaka",
    status: "ongoing",
    flatSize: "1130, 1420 sft",
    landArea: "5 Katha",
    floors: "G+9",
    facing: "South-East",
    amenities: ["Elevator", "Generator", "Parking", "Rooftop Garden", "Security"],
    description: "A premium residential project in the heart of Aftabnagar with modern amenities and elegant design.",
    descriptionBn: "আফতাবনগরের প্রাণকেন্দ্রে আধুনিক সুযোগ-সুবিধা ও মার্জিত নকশাসহ একটি প্রিমিয়াম আবাসিক প্রকল্প।",
    image: "/S & D Bondhon Tower/S & D Bondhon Tower Chandpur.jpg",
    images: [
      "/S & D Bondhon Tower/S & D Bondhon Tower Chandpur.jpg",
      "/S & D Bondhon Tower/S & D Bondhon Tower Chandpur Top.jpg",
      "/S & D Bondhon Tower/S & D Bondhon Tower Chandpur Night View-.jpg"
    ],
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Bondhon.Tower.pdf",
  },
  {
    slug: "laila-garden",
    name: "Laila Garden",
    nameBn: "লায়লা গার্ডেন",
    location: "Road-02, Block-C, Aftabnagar, Dhaka",
    locationBn: "রোড-০২, ব্লক-সি, আফতাবনগর, ঢাকা",
    city: "Dhaka",
    status: "completed",
    flatSize: "1594 sft",
    landArea: "6 Katha",
    floors: "G+6",
    facing: "South",
    amenities: ["Elevator", "Generator", "Parking", "Community Hall"],
    description: "A beautifully completed residential complex in Chandpur offering spacious living in a serene environment.",
    descriptionBn: "চাঁদপুরে একটি সুন্দরভাবে সম্পন্ন আবাসিক কমপ্লেক্স যা শান্ত পরিবেশে প্রশান্তিময় জীবনযাপনের সুযোগ দেয়।",
    image: "/S&D Laila Garden, Aftabnagar/S&D Laila Garden Aftabnagar.jpg",
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/S.D___Edit.pdf",
  },
  {
    slug: "fm-tower",
    name: "F.M Tower",
    nameBn: "এফ.এম টাওয়ার",
    location: "Opposite site of Chandpur Zonal Passport Office, Faridganj Road, Chandpur",
    locationBn: "চাঁদপুর জোনাল পাসপোর্ট অফিসের বিপরীতে, ফরিদগঞ্জ রোড, চাঁদপুর",
    city: "Chandpur",
    status: "ongoing",
    flatSize: "850, 1050 sft",
    landArea: "4.5 Katha",
    floors: "G+9",
    facing: "West",
    amenities: ["Elevator", "Generator", "Parking", "Gym", "Security"],
    description: "Modern urban living with premium finishes and strategic location near key amenities.",
    descriptionBn: "প্রধান সুবিধার কাছে কৌশলগত অবস্থান সহ প্রিমিয়াম ফিনিশিং সহ আধুনিক শহুরে জীবনযাপন।",
    image: "/S & D FM Tower/21_1 - Photo.jpg",
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/FM.Tower.Brochure.pdf",
  },
  {
    slug: "giash-garden",
    name: "Giash Garden",
    nameBn: "গিয়াশ গার্ডেন",
    location: "Road-02, Block-C, Aftabnagar, Dhaka",
    locationBn: "রোড-০২, ব্লক-সি, আফতাবনগর, ঢাকা",
    city: "Dhaka",
    status: "completed",
    flatSize: "1050 sft",
    landArea: "7 Katha",
    floors: "G+6",
    facing: "North-South",
    amenities: ["Elevator", "Generator", "Parking", "Playground", "Rooftop Garden"],
    description: "An upcoming landmark project in Chandpur with spacious apartments and lush green surroundings.",
    descriptionBn: "চাঁদপুরে প্রশস্ত অ্যাপার্টমেন্ট এবং সবুজ পরিবেশসহ একটি আসন্ন ল্যান্ডমার্ক প্রকল্প।",
    image: "/S & D Giash Garden/S&D View.jpg",
  },
  {
    slug: "safura-heights",
    name: "Safura Heights",
    nameBn: "সাফুরা হাইটস",
    location: "Road-06, Block-H, Aftabnagar, Dhaka",
    locationBn: "রোড-০৬, ব্লক-এইচ, আফতাবনগর, ঢাকা",
    city: "Dhaka",
    status: "completed",
    flatSize: "1650 sft",
    landArea: "5.5 Katha",
    floors: "G+6",
    facing: "East",
    amenities: ["Elevator", "Generator", "Parking", "Swimming Pool", "Gym", "Security"],
    description: "A prestigious completed project offering luxury living with world-class amenities.",
    descriptionBn: "বিশ্বমানের সুবিধাসহ বিলাসবহুল জীবনযাপনের প্রস্তাব দেওয়া একটি আধুনিক প্রকল্প।",
    image: "/S&D Safura Heights, Aftabnagar/1.6  Regular edited.jpg",
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Safura.Heights-.pdf",
  },
  {
    slug: "sardar-palace",
    name: "Sardar Palace",
    nameBn: "সর্দার প্যালেস",
    location: "Guakhula Road, Chandpur",
    locationBn: "গুয়াখোলা রোড, চাঁদপুর",
    city: "Chandpur",
    status: "completed",
    flatSize: "1210, 1250 sft",
    landArea: "6 Katha",
    floors: "G+6",
    facing: "South-West",
    amenities: ["Elevator", "Generator", "Parking", "Community Hall"],
    description: "A regal residential project reflecting elegance and comfort in the heart of Chandpur.",
    descriptionBn: "চাঁদপুর শহরের প্রাণকেন্দ্রে মনোরম পরিবেশে এবং আরামদায়াক ফ্ল্যাটের এক রাজকীয় আবাসিক প্রকল্প।",
    image: "/S & D Sardar Palace,  Chandpur/S & D Sardar Palace Chandpur.jpg",
    images: [
      "/S & D Sardar Palace,  Chandpur/S & D Sardar Palace Chandpur.jpg",
      "/S & D Sardar Palace,  Chandpur/S & D Sardar Palace Chandpur Top.jpg",
      "/S & D Sardar Palace,  Chandpur/S & D Sardar Palace Chandpur Top NIGHT VIEW.jpg"
    ],
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Sarder.Palace.Outline.pdf",
  },
  {
    slug: "didar-box",
    name: "Didar Box",
    nameBn: "দিদার বক্স",
    location: "Back side of Modina Shopping Center, Kalibari Mor, Chandpur",
    locationBn: "মদিনা শপিং সেন্টারের পেছনে, কালীবাড়ি মোড়, চাঁদপুর",
    city: "Chandpur",
    status: "ongoing",
    flatSize: "1205 sft",
    landArea: "4 Katha",
    floors: "G+6",
    facing: "East",
    amenities: ["Elevator", "Generator", "Parking", "Security"],
    description: "A modern upcoming project blending functionality with aesthetics.",
    descriptionBn: "নান্দনিকতার সাথে কার্যকারিতার মিশ্রণকারী একটি আধুনিক আসন্ন প্রকল্প।",
    image: "/S & D Didar Box/Final (1).jpg",
    images: ["/S & D Didar Box/Final (1).jpg"],
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Didar.Box.Brochure.Final.pdf",
  },
  {
    slug: "hossen-vill",
    name: "Hossain Villa",
    nameBn: "হোসাইন ভিলা",
    location: "Ukilpara Hardyal Road, Natun Bazar, Chandpur",
    locationBn: "উকিলপাড়া হরদয়াল রোড, নতুন বাজার, চাঁদপুর",
    city: "Chandpur",
    status: "ongoing",
    flatSize: "950, 1000 sft",
    landArea: "5 Katha",
    floors: "G+6",
    facing: "South",
    amenities: ["Elevator", "Generator", "Parking", "Security"],
    description: "A serene residential villa located in the heart of Chandpur.",
    descriptionBn: "চাঁদপুর শহরের প্রাণকেন্দ্রে অবস্থিত একটি শান্ত আবাসিক ভিলা।",
    image: "/S & D Hossen Vill/1_2 - Photo01.jpg",
    images: ["/S & D Hossen Vill/1_2 - Photo01.jpg"],
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Broshure.SND.Mithu.Chandpur.pdf",
  },
  {
    slug: "majeda-garden",
    name: "Majeda Garden",
    nameBn: "মাজেদা গার্ডেন",
    location: "Taltola, Chandpur",
    locationBn: "তালতলা, চাঁদপুর",
    city: "Chandpur",
    status: "completed",
    flatSize: "915, 970, 1060, 1140 sft",
    landArea: "6 Katha",
    floors: "G+9",
    facing: "North",
    amenities: ["Elevator", "Generator", "Parking", "Playground"],
    description: "A beautiful residential sanctuary offering lush gardens and premium facilities.",
    descriptionBn: "বিলাসবহুল বাগান এবং প্রিমিয়াম সুবিধা প্রদানকারী একটি সুন্দর আবাসিক অভয়ারণ্য।",
    image: "/S & D Majeda Garden, Chandpur/S & D Majeda Garden Chandpur.jpg",
    images: ["/S & D Majeda Garden, Chandpur/S & D Majeda Garden Chandpur.jpg"],
    brochure: "/S & D Majeda Garden, Chandpur/majeda broshior/1-16.jpg",
  },
  {
    slug: "nurjahan-palace",
    name: "Nurjahan Palace",
    nameBn: "নূরজাহান প্যালেস",
    location: "Jor Pukur Par, Chandpur",
    locationBn: "জোড় পুকুরপাড়, চাঁদপুর",
    city: "Chandpur",
    status: "completed",
    flatSize: "1298 sft",
    landArea: "5.5 Katha",
    floors: "G+6",
    facing: "West",
    amenities: ["Elevator", "Generator", "Parking", "Security"],
    description: "Experience royal living with modern amenities in Chandpur.",
    descriptionBn: "চাঁদপুরে আধুনিক সুযোগ-সুবিধা সহ রাজকীয় জীবনযাপনের অভিজ্ঞতা নিন।",
    image: "/S & D Nurjahan Palace, Chandpur/1_3 - Photo.jpg",
    images: ["/S & D Nurjahan Palace, Chandpur/1_3 - Photo.jpg"],
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Nurjahan.Palace.Outline.pdf",
  },
  {
    slug: "shohodora-palace",
    name: "Shohodora Palace",
    nameBn: "সহোদরা প্যালেস",
    location: "Nabinbag, Khilgaon, Dhaka",
    locationBn: "নবীনবাগ, খিলগাঁও, ঢাকা",
    city: "Dhaka",
    status: "upcoming",
    flatSize: "1385 sft",
    landArea: "4.5 Katha",
    floors: "G+6",
    facing: "South-East",
    amenities: ["Elevator", "Generator", "Parking", "Community Hall"],
    description: "A well-designed completed project offering comfort and security.",
    descriptionBn: "আরাম এবং নিরাপত্তা প্রদানকারী একটি সু-নকশাকৃত সম্পন্ন প্রকল্প।",
    image: "/S & D Shohodora Palace/141.png",
    images: ["/S & D Shohodora Palace/141.png"],
    brochure: "https://github.com/DelwarOfficial/sweet-home/releases/download/v1.0.0-assets/Broshure.Shohodora.Palace.pdf",
  },
  {
    slug: "rouf-garden",
    name: "Rouf Garden",
    nameBn: "রউফ গার্ডেন",
    location: "Block-F, Aftabnagar, Dhaka",
    locationBn: "ব্লক-এফ, আফতাবনগর, ঢাকা",
    city: "Dhaka",
    status: "upcoming",
    flatSize: "",
    landArea: "",
    floors: "G+8",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
  {
    slug: "sufia-monjil",
    name: "Sufia Monjil",
    nameBn: "সুফিয়া মঞ্জিল",
    location: "Plot-6228, Block-N, Road-52, Bashundhara (R/A), Dhaka",
    locationBn: "প্লট-৬২২৮, ব্লক-এন, রোড-৫২, বসুন্ধরা আবাসিক এলাকা, ঢাকা",
    city: "Dhaka",
    status: "upcoming",
    flatSize: "1600 sft",
    landArea: "",
    floors: "G+6",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
  {
    slug: "j-f-tower",
    name: "J.F Tower",
    nameBn: "জে.এফ টাওয়ার",
    location: "Block-D, Aftabnagar, Dhaka",
    locationBn: "ব্লক-ডি, আফতাবনগর, ঢাকা",
    city: "Dhaka",
    status: "ongoing",
    flatSize: "",
    landArea: "",
    floors: "G+8",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
  {
    slug: "zahir-mansion",
    name: "Zahir Mansion",
    nameBn: "জাহির ম্যানশন",
    location: "Plot-2005, Block-M, Road-30, Bashundhara(R/A), Dhaka",
    locationBn: "প্লট-২০০৫, ব্লক-এম, রোড-৩০, বসুন্ধরা আবাসিক এলাকা, ঢাকা",
    city: "Dhaka",
    status: "ongoing",
    flatSize: "2200 sft",
    landArea: "",
    floors: "G+8",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
  {
    slug: "rajia-mansion",
    name: "Rajia Mansion",
    nameBn: "রাজিয়া ম্যানশন",
    location: "Guakhula Road, Chandpur",
    locationBn: "গুয়াখোলা রোড, চাঁদপুর",
    city: "Chandpur",
    status: "upcoming",
    flatSize: "",
    landArea: "",
    floors: "G+7",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
  {
    slug: "sonali-kuitir",
    name: "Sonali Kuitir",
    nameBn: "সোনালী কুঠির",
    location: "Opposite site of DC Office & Chief Judicial Magistrate Court Chandpur Sadar, Chandpur",
    locationBn: "ডিসি অফিস ও চিফ জুডিশিয়াল ম্যাজিস্ট্রেট কোর্টের বিপরীতে, চাঁদপুর সদর, চাঁদপুর",
    city: "Chandpur",
    status: "ongoing",
    flatSize: "1290, 1302 sft",
    landArea: "",
    floors: "G+8",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
  {
    slug: "rahima-palace",
    name: "Rahima Palace",
    nameBn: "রাহিমা প্যালেস",
    location: "Opposite site of Superindent of Police Chandpur Sadar, Chandpur",
    locationBn: "পুলিশ সুপার কার্যালয়ের বিপরীতে, চাঁদপুর সদর, চাঁদপুর",
    city: "Chandpur",
    status: "ongoing",
    flatSize: "",
    landArea: "",
    floors: "G+6",
    facing: "",
    amenities: [],
    description: "",
    descriptionBn: "",
    image: "",
    images: [],
  },
];
